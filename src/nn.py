...
class SimpleNN: 
    ...
    def train(self, x_train: Tensor, y_train: Tensor, epochs: int = 100, learning_rate: SupportsFloat = 0.01): 
        """
        ...
        for epoch in range(epochs): 
            ...
            for param in [self.l1.weights, self.l1.biases, self.l2.weights, self.l2.biases]: 
...
if __name__ == "__main__":
    ...
    y_train = [[0], [1], [1], [0]]
    x_train = Tensor(np.array(x_train, dtype=np.float64), requires_grad=True)  # 10 samples, 3 features
    y_train = Tensor(np.array(y_train, dtype=np.float64), requires_grad=True)  # 10 samples, 2 output values
